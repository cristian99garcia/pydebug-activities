...
class GtkSourceviewPage(SearchablePage):
    ...
    def save(self,skip_md5 = False, interactive_close=False,new_file=None):
        ...
        if not self.fullPath.startswith(self._activity.storage):
            _logger.debug('failed to save self.fullPath: %s, Checked for starting with %s' % (self.fullPath, self._activity.debugger_home))
            self._activity.util.confirmation_alert(_('Would you like to include %s in your project?' % os.path.basename(self.fullPath)), _('This MODIFIED File is not in your package'), self.save_to_project_cb)
            return

        if interactive_close and self.text_buffer.can_undo():
            self._activity.util.confirmation_alert(_('Would you like to Save the file, or cancel and abandon the changes?'), _('This File Has Been Changed'),self.continue_save)
